    ...
    const showDataAddress = async () => {
      ...
      try {
        const response = await fetch('http://localhost:3000/api/address/byuser', {
          ...
          headers: {
              'Content-Type': 'application/json', // Đặt tiêu đề để chỉ định loại dữ liệu
          },
          body: JSON.stringify({
              // Dữ liệu bạn muốn gửi đi, có thể tùy chỉnh theo yêu cầu của API
              UserID: decodedToken.userId, // Ví dụ tham số userId
          }),
      });        
